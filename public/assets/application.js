...
(function() {
  var dataSource, map, selectTag, selectedTags, tags_handle;
  ...
  selectTag = function(tagName) {
    ...
    tag.find('button').click(function(event) {
      tag.remove();
      selectedTags.splice(selectedTags.indexOf(tagName), 1);
      return $('#tags').val(selectedTags.join(','));
    });
    return $('#tags_pool').append(tag);
  };

  tags_handle = function() {
    return $(document).ready(function() {
      dataSource = $('#tags_data_source').data('source');
      $.each($('#selected_tags').data('source'), function(i, selectedTag) {
        return selectTag(selectedTag);
      });
